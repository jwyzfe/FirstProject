...
'''
url 만 있으면 됨? 
daily로 쌓아서 모아가는 방법인게 나은가? 
파라미터는 url만 뻬면되나 

태그? 태그는 어차피 계속 바뀔 텐데? 
그냥 각 기사마다 날짜 보는게 최선일 듯? 
어차피 여기저기 기사 모아두는 느낌이고 리스트에 업데이트 하는 방식도 최신순 보다는 조회순이나 알고리즘 같은 느낌 

딱 정해진 시간에만 수행하면 되게끔 하면 되고, 
파라미터는 없는 케이스를 정하면 되려나 
'''
